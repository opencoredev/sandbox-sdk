...
Authenticated normalized adapters keep preview credentials inside `request()`. `toJSON()` never returns credential-bearing headers and redacts URL query values. Managed integrations that accept only a URL can return a credential-bearing URL; treat the result of `getPortUrl()` as sensitive.
...
| Blaxel         | Returns a public or token-authenticated preview URL.               |
