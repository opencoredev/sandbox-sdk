...
The Sandbox 2 happy path is the adapter factory:
...
`createSandbox({ provider })` stays for this major:
...
| `provider`     | `string`             | Built-in or community adapter id. |
...
`stop()` is idempotent, so repeated calls share the same cleanup operation. `await using` and `withSandbox()` call `stop()`, not `destroy()`.

Use `sandbox.destroy()` when the sandbox must be permanently deleted. That is a different operation from `stop()`: a stopped Vercel sandbox can keep persistent state; `destroy()` deletes it.
