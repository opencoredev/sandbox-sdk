...
await using sandbox = await daytona().create();
...
<ProviderAISDKExample provider="daytona" />
...
- `stop()` calls the native stop API when present. `destroy()` deletes the workspace.
