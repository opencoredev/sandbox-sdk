...
await using sandbox = await railway({ idleTimeoutMinutes: 5 }).create();
console.log((await sandbox.run("node --version")).stdout);
```

`stop()` releases the SDK wrapper and leaves the sandbox to its configured idle timeout. `destroy()` calls native destroy. The adapter is experimental. `liveTest` is `null`.
