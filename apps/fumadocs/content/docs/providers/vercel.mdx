...
await using sandbox = await vercel().create();
...
<ProviderAISDKExample provider="vercel" />
...
- `stop()` does not delete persistent state. `destroy()` does.
- `liveTest` is `null`. No dated live run is published.
