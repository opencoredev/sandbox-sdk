...
Eve requires a provider with managed session support. Railway and Cloudflare do not expose that surface, so the Eve adapter rejects them before creating a backend session.
...
<Tabs items={["Local", "E2B", "Daytona", "Vercel", "Upstash", "Ascii Box"]}>
  ...
  <Tab value="Ascii Box">
    ...
    const backend = createEveSandboxBackend({ provider: box() });
    ```
  </Tab>
</Tabs>
