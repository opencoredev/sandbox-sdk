---
title: Mastra
description: Run Mastra workspace agents on providers with managed session support.
icon: mastra
---
...
Each provider with managed session support uses the same `createMastraWorkspace()` factory. Railway and Cloudflare do not expose that surface, so the Mastra adapter rejects them before creating a workspace.

<Tabs items={["Local", "E2B", "Daytona", "Vercel", "Upstash", "Ascii Box"]}>
  <Tab value="Local">
    ```ts
    import { createMastraWorkspace } from "@opencoredev/sandbox-sdk/mastra";
    import { local } from "@opencoredev/sandbox-sdk/local";
